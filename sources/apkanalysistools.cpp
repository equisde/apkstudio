...
bool SSLPinningDialog::unpinLocation(const PinningLocation &location)
{
    ...
    // Enhanced unpinning based on type
    if (location.pinType == "trustmanager" || location.pinType == "certificate") {
        // For TrustManager implementations, we need to bypass the check
        if (line.contains("checkServerTrusted") || line.contains("checkClientTrusted")) {
            // Comment out and add return statement
        ...
        // For OkHttp CertificatePinner, replace with empty builder
        if (line.contains("CertificatePinner.Builder")) {
            // Keep the builder but remove add() calls in subsequent lines
            for (int i = location.lineNumber; i < qMin(lines.size(), location.lineNumber + 20); i++) {
                if (lines[i].contains(".add(")) {
                    lines[i] = "# UNPINNED: " + lines[i];
                    modified = true;
                } else if (lines[i].contains(".build()")) {
                    break;
                }
            }
        } else if (!line.trimmed().startsWith("#")) {
    ...
    } else if (location.pinType == "network_config") {
        // For network security config, modify XML to trust user certs
        if (line.contains("<pin-set")) {
            line = "<!-- UNPINNED: " + line.trimmed() + " -->";
            // Comment out until </pin-set>
            for (int i = location.lineNumber; i < lines.size(); i++) {
                if (!lines[i].trimmed().startsWith("<!--")) {
                    lines[i] = "<!-- " + lines[i] + " -->";
                }
                if (lines[i].contains("</pin-set>")) break;
            }
            modified = true;
        } else if (line.contains("<certificates src=\"system\"")) {
            // Add user certificates alongside system
            QString indent = line.left(line.indexOf('<'));
            lines.insert(location.lineNumber, indent + "<certificates src=\"user\"/>");
            modified = true;
        }
    } else if (location.pinType == "ssl_context") {
        // For custom SSLContext, comment out and note the bypass
        if (!line.trimmed().startsWith("#")) {
            line = "# UNPINNED (SSLContext): " + line;
            modified = true;
        }
    }
...
{
    QString script = R"(// Frida SSL Pinning Bypass Script - Universal
// Generated by APK Studio
// Covers: OkHttp3, OkHttp, Retrofit, TrustManager, SSLContext, WebView, 
//         Certificate Transparency, Conscrypt, Apache HTTP, and more
...
Java.perform(function() {
    ...
    try {
        ...
        CertificatePinner.check.overload('java.lang.String', 'java.util.List').implementation = function(hostname, peerCertificates) {
            console.log("[+] OkHttp3 CertificatePinner.check(String,List) bypassed: " + hostname);
            return;
        };
    } catch(e) { console.log("[-] OkHttp3 check(String,List): " + e.message); }
    ...
    try {
        ...
        CertificatePinner.check.overload('java.lang.String', '[Ljava.security.cert.Certificate;').implementation = function(hostname, peerCertificates) {
            console.log("[+] OkHttp3 CertificatePinner.check(String,Certificate[]) bypassed: " + hostname);
            return;
        };
    } catch(e) { console.log("[-] OkHttp3 check(String,Certificate[]): " + e.message); }

    try {
        var CertificatePinner = Java.use("okhttp3.CertificatePinner");
        CertificatePinner['check$okhttp'].implementation = function(hostname, cleanFun) {
            console.log("[+] OkHttp3 check$okhttp bypassed: " + hostname);
            return;
        };
    } catch(e) { console.log("[-] OkHttp3 check$okhttp: " + e.message); }
    ...
    try {
        ...
        CertPinner.check.overload('java.lang.String', 'java.util.List').implementation = function(hostname, peerCerts) {
            console.log("[+] OkHttp CertificatePinner bypassed: " + hostname);
            return;
        };
    } catch(e) { console.log("[-] OkHttp CertificatePinner: " + e.message); }
    ...
    } catch(e) { console.log("[-] TrustManagerImpl.verifyChain: " + e.message); }
    ...
    try {
        ...
        TrustManagerImpl.checkTrustedRecursive.implementation = function(certs, ocspData, tlsSctData, host, clientAuth, untrustedChain, trustAnchorChain, used) {
            console.log("[+] TrustManagerImpl.checkTrustedRecursive bypassed: " + host);
            return Java.use("java.util.ArrayList").$new();
        };
    } catch(e) { console.log("[-] TrustManagerImpl.checkTrustedRecursive: " + e.message); }
    
    // ==================== X509TrustManager ====================
    try {
        var X509TrustManager = Java.use("javax.net.ssl.X509TrustManager");
        var TrustManager = Java.registerClass({
            name: "com.apkstudio.TrustAll",
    ...
    } catch(e) { console.log("[-] X509TrustManager registration: " + e.message); }
    ...
    try {
        ...
        SSLContext.init.overload('[Ljavax.net.ssl.KeyManager;', '[Ljavax.net.ssl.TrustManager;', 'java.security.SecureRandom').implementation = function(km, tm, sr) {
            ...
            var TrustAllManager = Java.use("com.apkstudio.TrustAll");
            var emptyTM = TrustAllManager.$new();
            var tmArray = Java.array('javax.net.ssl.TrustManager', [emptyTM]);
            this.init(km, tmArray, sr);
        };
    } catch(e) { console.log("[-] SSLContext.init: " + e.message); }
    
    // ==================== HostnameVerifier ====================
    try {
        var HostnameVerifier = Java.use("javax.net.ssl.HostnameVerifier");
        var AllowAll = Java.registerClass({
            name: "com.apkstudio.AllowAllHostnameVerifier",
    ...
    } catch(e) { console.log("[-] HostnameVerifier: " + e.message); }
    ...
    } catch(e) { console.log("[-] HttpsURLConnection verifier: " + e.message); }
    ...
    } catch(e) { console.log("[-] WebViewClient: " + e.message); }
    
    // ==================== Appcelerator Titanium ====================
    try {
        var PinningTrustManager = Java.use("appcelerator.https.PinningTrustManager");
        PinningTrustManager.checkServerTrusted.implementation = function(chain, authType) {
            console.log("[+] Titanium PinningTrustManager bypassed");
            return;
        };
    } catch(e) { console.log("[-] Titanium: " + e.message); }
    
    // ==================== PhoneGap/Cordova ====================
    try {
        var Whitelist = Java.use("org.apache.cordova.CordovaWebViewEngine$WhitelistHandler");
        Whitelist.isUrlWhiteListed.implementation = function(url) {
            console.log("[+] Cordova WhitelistHandler bypassed: " + url);
            return true;
        };
    } catch(e) { console.log("[-] Cordova: " + e.message); }
    
    // ==================== Flutter (io.flutter.plugins.flutter_plugin_android_lifecycle) ====================
    try {
        var SslPinningPlugin = Java.use("io.flutter.plugins.urllauncher.UrlLauncherPlugin");
        // Flutter apps may use different packages
    } catch(e) { console.log("[-] Flutter: " + e.message); }
    
    // ==================== Conscrypt ====================
    try {
        var OpenSSLSocketImpl = Java.use("com.android.org.conscrypt.OpenSSLSocketImpl");
        OpenSSLSocketImpl.verifyCertificateChain.implementation = function(certRefs, javaCerts, authMethod) {
            console.log("[+] Conscrypt verifyCertificateChain bypassed");
            return;
        };
    } catch(e) { console.log("[-] Conscrypt verifyCertificateChain: " + e.message); }
    ...
    try {
        ...
        ConscryptFileDescriptorSocket.verifyCertificateChain.implementation = function(certRefs, authMethod) {
            console.log("[+] ConscryptFileDescriptorSocket bypassed");
            return;
        };
    } catch(e) { console.log("[-] ConscryptFileDescriptorSocket: " + e.message); }
    ...
    try {
        ...
        DefaultHostnameVerifier.verify.overload('java.lang.String', 'javax.net.ssl.SSLSession').implementation = function(host, session) {
            console.log("[+] Apache DefaultHostnameVerifier bypassed: " + host);
            return true;
        };
    } catch(e) { console.log("[-] Apache HTTP: " + e.message); }
    
    // ==================== CWAC-NetSecurity ====================
    try {
        var CertChainBuilder = Java.use("com.commonsware.cwac.netsecurity.TrustManagerBuilder$CertChainBuilder");
        CertChainBuilder.checkServerTrusted.implementation = function(certs, authType) {
            console.log("[+] CWAC CertChainBuilder bypassed");
            return;
        };
    } catch(e) { console.log("[-] CWAC: " + e.message); }
    
    // ==================== Network Security Config Trust Anchors ====================
    try {
    ...
    } catch(e) { console.log("[-] NetworkSecurityConfig: " + e.message); }
    
    console.log("[*] Universal SSL Pinning Bypass Complete - All hooks installed");
});
)";
    ...
    if (!savePath.isEmpty()) {
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            QMessageBox::information(this, tr("Script Saved"),
                tr("Universal Frida SSL Bypass script saved to:\n%1\n\n"
                   "Usage:\n"
                   "  frida -U -f <package> -l frida_ssl_bypass.js --no-pause\n\n"
                   "Or with objection:\n"
                   "  objection -g <package> explore\n"
                   "  android sslpinning disable").arg(savePath));
        }
    }
}
